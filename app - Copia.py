        ...
        try:
            ...
            if params_doc:
                g.parametros_globais = {
                    ...
                    'comissao_padrao':   params_doc.get('comissao_padrao'),   
                }
        except Exception as e:
...
def cadastro_colaborador():
    ...
    # --- INÍCIO DA NOVA LÓGICA (Passar comissão padrão) ---
    # Pega a comissão padrão de 'g' (carregada no before_request)
    # Define 20 como um fallback de segurança se não encontrar
    ...
    context = {
        ...
        'query': search_term, 
        'colaborador_edicao': colaborador_edicao, # Esta é a variável 'colab' no seu HTML
        'error': error,
        'success': success,
        'g': g,
        'default_comissao': default_comissao # <-- ADICIONADO AQUI
    }
...
def gravar_colaborador():
    ...
    try:
        ...
        comissao = int(request.form.get('comissao'))
...
# --- ROTAS DE CONSULTA DE VENDAS (ATUALIZADAS COM CÁLCULO DE COMISSÃO) ---
@app.route('/consulta_vendas', methods=['GET'])
@login_required
def consulta_vendas():
    """
    Página principal de consulta de vendas.
    (ATUALIZADO: Agora calcula e exibe a comissão do colaborador)
    """
    db = g.db
    if not g.db_status:
    ...
    # --- INÍCIO DA LÓGICA DE COMISSÃO ---
    # Helper Interno para limpar dados do evento
    ...
    # Pega a comissão padrão de 'g' (carregada no before_request)
    default_comissao = g.parametros_globais.get('comissao_padrao', 0) # Padrão 0 se não definida
    # --- FIM DA LÓGICA DE COMISSÃO ---

    try:
        # 4. Lógica de Carregamento da Página
        
        # Etapa A: Se nenhum evento foi selecionado...
        ...
        # Etapa B: Se um evento FOI selecionado...
            ...
            resultados_temp = list(resultados_cursor) # Converte para lista
            
            # --- 6. OTIMIZAÇÃO DE COMISSÃO ---
            # 6a. Pega todos os IDs de colaborador dos resultados
            colab_ids = [r['_id'] for r in resultados_temp if isinstance(r['_id'], int)]
            
            # 6b. Faz UMA consulta para pegar todas as comissões
            comissao_map = {}
            if colab_ids:
                colabs_docs = db.colaboradores.find(
                    {'id_colaborador': {'$in': colab_ids}},
                    {'id_colaborador': 1, 'comissao': 1}
                )
                # Cria um mapa de ID -> comissao
                for c in colabs_docs:
                    taxa = c.get('comissao')
                    if isinstance(taxa, (int, float)):
                        comissao_map[c['id_colaborador']] = taxa
                    else:
                        comissao_map[c['id_colaborador']] = default_comissao

            # 7. Formata os resultados e CALCULA comissão
            for res in resultados_temp:
                res['total_valor_float'] = safe_float(res['total_valor'])
                
                # --- CÁLCULO DE COMISSÃO ---
                colab_id = res['_id']
                # Pega a taxa do mapa, ou usa o padrão se o colaborador não tiver
                taxa_comissao = comissao_map.get(colab_id, default_comissao) 
                
                # Calcula (ex: (1000.00 * 20) / 100)
                valor_comissao = (res['total_valor_float'] * taxa_comissao) / 100.0
                res['valor_comissao_float'] = valor_comissao
                res['taxa_comissao_aplicada'] = taxa_comissao # Para debug
                # --- FIM DO CÁLCULO ---
...
# Por favor, substitua sua rota 'consulta_vendas_detalhes' existente por esta:
    ...
    try:
        ...
        if nivel_usuario < 3:
            ...
            if id_colaborador_param and id_colaborador_param != 'ALL':
                ...
                # Se for TODOS, vamos buscar as comissões de *todos* os colaboradores
        ...
        # 4. Otimização: Busca comissões se for um colab específico
        if colab_ids_para_buscar_comissao:
             colab_doc = db.colaboradores.find_one(
        ...
        for venda in vendas_cursor:
            ...
            # Calcula comissão para esta venda
